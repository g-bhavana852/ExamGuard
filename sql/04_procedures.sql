...
-- SP4  sp_log_proctor_event    Front-end logs a proctoring event
-- SP5  sp_get_exam_results     Student views their results (2 result sets)
-- SP6  sp_get_flagged_attempts Proctor dashboard
-- SP7  sp_resolve_flag         Proctor closes a suspicion flag
-- SP8  sp_exam_analytics       Instructor summary report (3 result sets)
...
-- creates ExamAttempts row, logs the start event.
...
sp_start_exam: BEGIN
    ...
    BEGIN
        ...
        ROLLBACK; SET p_attempt_id = NULL;
    END;
...
-- Upsert: INSERT on first answer, UPDATE on revisit.
-- T2 (time check) and T3 (auto-grade) fire on every INSERT.
-- UPDATE branch re-grades manually since T3 is INSERT-only.
...
BEGIN
    ...
    START TRANSACTION;
        INSERT INTO StudentAnswers (attempt_id, question_id, selected_option, time_taken_seconds, is_correct, marks_obtained)
        VALUES (p_attempt_id, p_question_id, p_selected, p_time_taken, v_is_correct, v_marks_obtained)
...
-- ── SP3 : sp_submit_exam ─────────────────────────────────────
-- Locks attempt row (FOR UPDATE), sums marks, updates status.
...
-- Lightweight wrapper; T4 updates suspicion_score, T5 auto-flags.
CREATE PROCEDURE sp_log_proctor_event(
    IN p_attempt_id INT, IN p_event_type VARCHAR(50),
    IN p_severity   VARCHAR(10), IN p_details TEXT, IN p_ip VARCHAR(45)
)
BEGIN
    INSERT INTO ProctorLogs (attempt_id, event_type, severity, event_details, ip_address)
...
-- RS1: Attempt summary. RS2: Per-question breakdown.
-- correct_answer shown only if show_results_immediately = TRUE.
CREATE PROCEDURE sp_get_exam_results(IN p_attempt_id INT, IN p_student_id INT)
BEGIN
    SELECT
        ea.attempt_id, e.title AS exam_title, c.course_name,
...
BEGIN
    ...
    WHERE  ea.exam_id = p_exam_id AND (ea.status = 'flagged' OR ea.suspicion_score >= 40)
...
BEGIN
    ...
    FROM   SuspicionFlags WHERE flag_id = p_flag_id AND is_resolved = FALSE;
...
-- RS1: Exam statistics. RS2: Question difficulty. RS3: Top 10 suspicious.
...
BEGIN
    SELECT
        ...
        ROUND(AVG(score), 2) AS avg_score, MAX(score) AS highest_score, MIN(score) AS lowest_score,
        ROUND(AVG(percentage), 2) AS avg_percentage,
        SUM(CASE WHEN score >= (SELECT passing_marks FROM Exams WHERE exam_id = p_exam_id) THEN 1 ELSE 0 END) AS pass_count,
        ROUND(100.0 * SUM(CASE WHEN score >= (SELECT passing_marks FROM Exams WHERE exam_id = p_exam_id) THEN 1 ELSE 0 END)
              / NULLIF(COUNT(*), 0), 1) AS pass_rate_pct
    FROM  ExamAttempts
    WHERE exam_id = p_exam_id AND status IN ('submitted', 'graded', 'flagged');

    SELECT
        q.question_id, LEFT(q.question_text, 60) AS question_preview,
        ...
        ROUND(100.0 * SUM(CASE WHEN sa.is_correct = TRUE THEN 1 ELSE 0 END)
              / NULLIF(COUNT(sa.answer_id), 0), 1) AS correct_pct
    FROM  Questions q
    ...
    ORDER BY ea.suspicion_score DESC LIMIT 10;
END //
