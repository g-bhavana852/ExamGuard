...
-- Shows all exams a student is eligible for + their attempt status.
-- ============================================================
SELECT
    e.exam_id,
    ...
    ea.submitted_at,
    CASE
        WHEN ea.attempt_id IS NULL AND NOW() BETWEEN e.window_start AND e.window_end
             THEN 'CAN START'
        WHEN ea.attempt_id IS NULL AND NOW() < e.window_start
             THEN 'UPCOMING'
        WHEN ea.attempt_id IS NULL
             THEN 'WINDOW CLOSED'
        ELSE 'ATTEMPTED'
    END                                               AS availability
...
WHERE  enr.student_id = 6           -- Arjun Kumar
...
-- Sorted by composite risk score for prioritisation.
-- ============================================================
SELECT
    ea.attempt_id,
    ...
    ea.score,
    ea.percentage,
    -- Composite risk: weight different indicators
    (ea.suspicion_score * 1.0
     + ea.tab_switches  * 3
     + ea.copy_paste_attempts * 5
     + COALESCE(SUM(pl.severity = 'CRITICAL'), 0) * 10
...
GROUP  BY ea.attempt_id, u.full_name, u.email, ex.title,
          ...
          ea.copy_paste_attempts, ea.face_not_detected,
          ea.fullscreen_exits, ea.score, ea.percentage
...
-- Questions with lowest correct-rate = most problematic.
-- ============================================================
SELECT
    q.question_id,
    LEFT(q.question_text, 70)                     AS question_preview,
    q.difficulty_level,
    q.marks,
    ...
    ROUND(
        100.0 * SUM(sa.is_correct = TRUE) / NULLIF(COUNT(sa.answer_id), 0), 1
    )                                             AS correct_pct,
    CASE
        WHEN ROUND(100.0 * SUM(sa.is_correct = TRUE) / NULLIF(COUNT(sa.answer_id), 0), 1) >= 80
             THEN 'Too Easy'
        WHEN ROUND(100.0 * SUM(sa.is_correct = TRUE) / NULLIF(COUNT(sa.answer_id), 0), 1) BETWEEN 40 AND 79
             THEN 'Good'
        ELSE 'Very Hard'
...
-- ============================================================
-- Q05 : Suspicion Leaderboard — Top Risk Students
-- Ranks students by risk across all attempted exams.
-- ============================================================
SELECT
    ...
    SUM(ea.face_not_detected)                 AS total_face_not_detected,
...
-- ============================================================
-- Q06 : Rapid Answering Detection
-- Students who answered questions suspiciously fast (< 15s avg)
-- ============================================================
SELECT
    u.full_name,
    ea.attempt_id,
...
HAVING avg_seconds_per_question < 15            -- threshold: under 15s per question is suspicious
ORDER  BY avg_seconds_per_question ASC;


-- ============================================================
-- Q07 : Multiple Login Report
-- Sessions where the same user was active from 2+ IPs concurrently.
-- ============================================================
SELECT
    u.full_name,
    u.email,
    ls1.ip_address                AS session_1_ip,
    ls2.ip_address                AS session_2_ip,
    ls1.login_time                AS session_1_start,
    ls2.login_time                AS session_2_start,
    ls1.device_fingerprint        AS device_1,
    ls2.device_fingerprint        AS device_2,
    ea.attempt_id,
    ea.status                     AS exam_status
FROM   LoginSessions ls1
JOIN   LoginSessions ls2
       ON  ls1.user_id    = ls2.user_id
       AND ls1.session_id < ls2.session_id        -- avoid self-join duplicates
       AND ls1.ip_address <> ls2.ip_address       -- different IPs
...
-- ============================================================
-- Q08 : Exam Completion Funnel
-- For each exam: enrolled → started → submitted → passed
-- ============================================================
SELECT
    ex.exam_id,
    ex.title,
    ...
    ROUND(
        100.0 * COUNT(DISTINCT ea.student_id) / NULLIF(COUNT(DISTINCT enr.student_id), 0), 1
    )                                        AS start_rate_pct,
    ROUND(
        100.0 * SUM(ea.score >= ex.passing_marks)
        / NULLIF(COUNT(DISTINCT ea.student_id), 0), 1
    )                                        AS pass_rate_pct
FROM   Exams ex
...
-- ============================================================
-- Q09 : Proctoring Event Timeline for a Specific Attempt
-- Full chronological event log with severity colour bands.
-- ============================================================
SELECT
    pl.log_id,
    pl.logged_at,
...
FROM   ProctorLogs pl
WHERE  pl.attempt_id = 1003           -- Ravi's suspicious attempt
...
SELECT
    ...
    -- Window function: rank within course
    RANK() OVER (ORDER BY AVG(ea.percentage) DESC)    AS `class_rank`
FROM   ExamAttempts ea
JOIN   Users  u  ON ea.student_id = u.user_id
JOIN   Exams  ex ON ea.exam_id    = ex.exam_id
WHERE  ex.course_id = 1            -- DBMS course
...
-- Most common proctoring event types ranked by frequency.
-- ============================================================
SELECT
    pl.event_type,
    pl.severity,
...
SELECT
    ...
    ROUND(
        100.0 * SUM(ea.score >= ex.passing_marks)
        / NULLIF(COUNT(ea.attempt_id), 0), 1
    )                                                 AS pass_rate_pct,
    ROUND(AVG(ea.percentage), 1)                      AS avg_percentage,
    ROUND(AVG(ea.suspicion_score), 1)                 AS avg_suspicion
FROM   Courses      c
